...
class add: public Operator<T> {
  int CHILD_ID = 1;
  public:
    add(Node<T> &a, Node<T> &b) : Operator<T>(a, b){setOP_ID();}
...
class sub: public Operator<T> {
  int CHILD_ID = 2;
  public:
    sub(Node<T> &a, Node<T> &b) : Operator<T>(a, b){setOP_ID();}
  ...
  public:
      ...
      void reset()
      {
        for(auto elem: ordering) { 
          if(elem->isPlaceholder == false) {
            Operator<T> * curr_op_parent =static_cast<Operator<T> *>(elem);
            curr_op_parent->isComputed = false;
          }
        }
      }
      ...
      T diff(Variable<T> &wrt_v) {
        ...
        if(most_recent_diff != &wrt_v)
        {
          reset();
          most_recent_diff = &wrt_v; 
        }
        // return DiffEngine(ordering,wrt_v);
        return diff(wrt_v.get_unq_node_idx());
        // std::cout << " Value of wrt_v: " << wrt_v.getValue() << std::endl;
      }


      T diff_one(Variable<T> &wrt_v, T curr_val) {
        // Assumes that there is only ONE Variable in the CG and hence resets the whole CG since every node would be reachable from it.
        reset();
        wrt_v.setValue(curr_val);
        return DiffEngine(ordering,wrt_v);
      }

      T diff(int wrt_idx)
      {
        return output_node->diff(wrt_idx);
      }
      ...
      // {
      //   for(auto elem: ordering){
      //      Operator<T> * temp = static_cast<Operator<T> *>(elem);
      //     temp->isComputed = false;
      //     temp->isDiffComputed = false;
      //   }
      // }
};
...
class ConstantFolding_PASS: public AD_PASS{
  public: 
    template<typename T>
    ConstantFolding_PASS(Expression<T> &E){}
};
...
int main(){
  Constant<fpm::fixed_16_16> c(1);
  Constant<float> d(8);
  Variable<fpm::fixed_16_16> v(9);
  std::cout << "Constant: " << c <<"\n";
  std::cout << "Variable: " << v << "\n";
  // std::cout << "Addition C+V: " << c+v << "\n";

  // Expression<fpm::fixed_16_16> z;
  // z <<= c/v*(c+v)*v*v;
  // std::cout << "Expression evaluation: "<< z.getValue() << "\n";
  // z.print_top_ordered_dbg();
  // std::map<Variable<fpm::fixed_16_16> *, fpm::fixed_16_16>  m1= {{&v,fpm::fixed_16_16{5}}};
